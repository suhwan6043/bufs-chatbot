...
export default function AdminLayout({ children, params }: { children: React.ReactNode; params: Promise<{ lang: string }> }) {
  ...
  const isLoginPage = pathname === `/${lang}/admin` || pathname === `/${lang}/admin/`;

  useEffect(() => { setMounted(true); }, []);

  // 미인증 + 서브페이지 → 로그인으로 리다이렉트
  useEffect(() => {
    if (!mounted || token) return;
    if (!isLoginPage) {
      router.replace(`/${lang}/admin`);
    }
  }, [mounted, token, isLoginPage, lang, router]);

  // SSR 깜빡임 방지
  if (!mounted) return null;

  // 미인증: 로그인 페이지만 렌더, 서브페이지는 완전 차단
  if (!token) {
    ...
    <div className="min-h-screen bg-main flex">
      ...
      <aside className={`fixed md:static z-40 w-56 h-screen bg-white border-r border-border flex flex-col transition-transform ${open ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
        ...
        <nav className="flex-1 py-2 overflow-y-auto">
          {MENU.map((m) => (
            <a key={m.key} href={`/${lang}/admin/${m.key}`}
        ...
        <div className="px-4 py-3 border-t border-border">
          <button onClick={() => { logout(); window.location.href = `/${lang}/admin`; }}
            className="w-full py-2 text-xs text-red-500 border border-red-200 rounded-lg hover:bg-red-50 transition-colors">
            로그아웃
          </button>
        </div>
      </aside>
