...
기능적·로직적 검증이 끝나면 **즉시 커밋·푸시**한다. 사용자가 따로 지시하지 않아도 된다.
...
4. **정답률 체크 (필수, 절대 생략 금지)**:
   ...
   - 4/16 기준선 (`reports/eval_contains_f1/combined_slicing_off_20260416_002907.json` = 81.7%)과 비교
...
검증 완료 → git add (선별, 스크래치·바이너리 제외)
         ...
         → 사용자에게 commit hash + 핵심 변경 요약 보고
```

- 사용자 명시적 승인 없이도 위 검증을 모두 통과했으면 커밋·푸시 진행한다 (검증 자체가 승인을 대체).
